...
public class PointCPTest
{
	...
	public static void main(String[] args)
	{
		...
		PointCPD2 pPoint= new PointCPD2(coords[0], coords[1]);
		...
		try
		{
			pPoint = new PointCPD2(Double.valueOf(args[1]).doubleValue(), 
					Double.valueOf(args[2]).doubleValue());
			cPoint = new PointCPD3(Double.valueOf(args[1]).doubleValue(), 
					Double.valueOf(args[2]).doubleValue());
		}
		catch(Exception e)
		{
			...
			try
			{
				...
				if(coordType != 'P')
				{
					cPoint = new PointCPD3(coords[0], coords[1]);
				}else {
		...
		if(coordType != 'P')
		{
			...
			System.out.println("Convert to Polar:\n" + "[" + cPoint.getRho() + "," + cPoint.getTheta() + "]\n");
		} else {
			System.out.println("\nYou entered:\n" + pPoint);
		...
		if(coordType == 'P') {
			runtimeTestD2(30, 200000, coordType);
		}else {
			runtimeTestD3(30, 200000, coordType);
		}
	...
	/**
	 * This method will create random PointCPD2 instances and run each method thousands of times.
	 *
	 * @param  sampleSize Sample size
	 * @param  iterations How many times of iterations for each methods
	 */
	private static void runtimeTestD2(int sampleSize, int iterations, char coordType)
	{
		...
		showTestResult(runtime, iterations, sampleSize, coordType);
	}
	 ...
	 * @param  iterations How many times of iterations for each methods
	 */
	private static void runtimeTestD3(int sampleSize, int iterations, char coordType)
	{
		System.out.println("Start Testing...");
		System.out.println("-----------------------------------------");
		int sampleNo = 0;
		...
		double[] rotation = new double[sampleSize];
		long[][] runtime = new long[sampleSize][9];
		PointCPD3 origin = new PointCPD3(0, 0);
		PointCPD3[] samples = new PointCPD3[sampleSize];
		...
		for(int i = 0; i<sampleSize; i++) {
			...
			double randD2 = rand.nextDouble()*10000; // Random Y from [0, 10000]
			double degree = rand.nextDouble()*10000; // Random degree from [0, 10000]
			samples[i] = new PointCPD3(randD1, randD2);
			rotation[i] = degree;
		}
	 ...
	 */
	private static void showTestResult(long[][] samples, int iterations, int sampleSize, char coordType) {
		...
		System.out.println("\nTest runs on "+sampleSize + ((coordType == 'C')?(" PointCPD3"):(" PointCPD2")) + " samples with "+iterations+" iterations for each methods:");
