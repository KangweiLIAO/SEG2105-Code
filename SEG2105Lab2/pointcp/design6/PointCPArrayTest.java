...
public class PointCPArrayTest
{
	...
	public static void main(String[] args)
	{
		...
		arrayListTestD2(12000000);
		System.out.println();
		arrayListTestD3(12000000);
	}
	 ...
	 */
	private static void arrayListTestD2(int sampleSize)
	{
	 ...
	 * @param  iterations How many times of iterations for each methods
	 */
	private static void arrayListTestD3(int sampleSize)
	{
		System.out.println("Start Testing with " + sampleSize + " samples...");
		System.out.println("-----------------------------------------");
		Random rand = new Random();
		...
		PointCPD3[] samplesOrdinary = new PointCPD3[sampleSize];
		ArrayList<PointCPD3> samplesArray = new ArrayList<PointCPD3>();
		LinkedList<PointCPD3> samplesLinked = new LinkedList<PointCPD3>();
		...
		for(int i = 0; i<sampleSize; i++) {
			double randD1 = rand.nextDouble()*9; // Random X from [0, 9]
			double randD2 = rand.nextDouble()*9; // Random Y from [0, 9]
			samplesArray.add(new PointCPD3(randD1, randD2));
		}
		end = System.currentTimeMillis();
		times[0] = end-start;
		...
		for(int i = 0; i<sampleSize; i++) {
			double randD1 = rand.nextDouble()*9; // Random X from [0, 9]
			double randD2 = rand.nextDouble()*9; // Random Y from [0, 9]
			samplesLinked.add(new PointCPD3(randD1, randD2));
		}
		end = System.currentTimeMillis();
		times[1] = end-start;
		...
		for(int i = 0; i<sampleSize; i++) {
			double randD1 = rand.nextDouble()*9; // Random X from [0, 9]
			double randD2 = rand.nextDouble()*9; // Random Y from [0, 9]
			samplesOrdinary[i] = new PointCPD3(randD1, randD2);
		}
		end = System.currentTimeMillis();
		times[2] = end-start;
		
		Iterator<PointCPD3> iteratorA = samplesArray.iterator();
		Iterator<PointCPD3> iteratorL = samplesLinked.iterator();
		//Test ArrayList Iterator
		start = System.currentTimeMillis();
		while(iteratorA.hasNext()) {
			iteratorA.next();
